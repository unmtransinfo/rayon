.. rayon documentation master file, created by Joel Berendzen on Aug 30, 2017.

Lorax "Speaks for the Trees"
================================
``rayon`` is a Flask-based web service that uses asynchronous queues (via RQ) to
calculate and serve up multiple sequence alignments and phylogenetic trees.
``rayon`` uses pre-calculated Hidden-Markov Models (HMM) of protein families
together with ``hmmalign`` to
calculate multiple sequence alignments in either peptide or DNA space.

``rayon`` calculates phylogenetic trees from either DNA or protein sequences by external
...
software that ``rayon`` knows about are:
