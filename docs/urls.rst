URLs
----
``rayon`` interacts through standard http ``GET`` and ``POST`` commands.  POST of
sequence data and calculation of trees results in permanent URLs that ``rayon`` will serve up
...
``/trees/<family>/sequences``       ``POST`` a FASTA-formatted set of aligned sequences.
                                    ...
                                    ''post_to_rayon.sh`` script in the test/ directory
