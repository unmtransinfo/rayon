...
SEQUENCE_EXTENSIONS = OrderedDict([("DNA", ".fna"), ("peptide", ".faa")])
...
# hmmalign stuff.
HMM_SWITCHES = {"peptide": "amino", "DNA": "dna"}
#
# Application data.
#
MAINTAINER = "Joel Berendzen"
GIT_REPO = "https://github.com/unmtransinfo/rayon"
...
app = Flask(
    __name__,
    instance_path=os.getenv(__name__.split(".")[0].upper() + "_ROOT", prefix),
...
def application_data():
    return {"maintainer": MAINTAINER, "git_repo": GIT_REPO}
...
def create_fasta(familyname, data_name, superfamily=None):
    ...
    try:  # parse FASTA file
        ...
        app.logger.error(
            'Unparseable/empty FASTA requested for family "%s".', familyname
        )
        abort(406)
...
def run_subprocess_with_status(
    out_path, err_path, cmdlist, cwd, status_path, post_process, post_args
):
    ...
    if post_process is not None:
        post_process(out_path, err_path, cwd, status, *post_args)
...
def convert_stockholm_to_fasta(out_path, err_path, cwd, status, fasta):
...
def queue_calculation(familyname, calculation, superfamily=None):
    ...
    if tree_builder is not None:  # will build a tree.
        ...
        if alignment_tool is None:  # build tree with alignment already done
            for key in SEQUENCE_EXTENSIONS.keys():
                if (
                    alignment_dir / (ALIGNMENT_NAME + SEQUENCE_EXTENSIONS[key])
                ).exists():
                    alignment_input_path = Path("..") / (
                        ALIGNMENT_NAME + SEQUENCE_EXTENSIONS[key]
                    )
    ...
    if alignment_tool is not None and tree_builder is not None:
        ...
        set_job_description(
            "alignment", alignment_tool, align_job, familyname, superfamily
        )
        tree_job = tree_queue.enqueue(
            run_subprocess_with_status,
            args=(
                raw_tree_path,
                tree_log_path,
...
def return_log():
    ...
    """
    content = get_file(__name__ + "_server.log", file_type="log")
...
def post_or_get_alignment(family):
    ...
    if request.method == "POST":
        ...
        alignment_path = Path(app.config["DATA"]) / family / ALIGNMENT_NAME
...
def post_or_get_alignment_superfamily(family, superfamily):
    ...
    if request.method == "POST":
        ...
        alignment_path = (
            Path(app.config["DATA"]) / family / superfamily / ALIGNMENT_NAME
        )
        for ext in SEQUENCE_EXTENSIONS.keys():
            test_path = alignment_path.with_suffix(SEQUENCE_EXTENSIONS[ext])
            if test_path.exists():
...
def put_hmm(family):
    ...
    for line in io.StringIO(hmmstats_output):
        if line.startswith("#") or line.startswith("\n"):
            ...
            except (TypeError, KeyError, ValueError):  # pragma: no cover
                app.logger.error(
...
def bind_calculation(method, superfamily=False):
    ...
    if not superfamily:
        ...
        def _calculate(family):
            return queue_calculation(family, method)
        ...
        def _calculate(family, sup):
            return queue_calculation(family, method, superfamily=sup)
...
def get_phyloxml_tree(familyname, method):
    if method not in app.config["TREEBUILDERS"]:
        abort(404)
    inpath = Path(app.config["DATA"]) / familyname / method / PHYLOXML_NAME
    if not inpath.exists():
        abort(404)
    return Response(inpath.open().read(), mimetype=NEWICK_MIMETYPE)
...
def get_log(familyname, method):
    ...
    if method in list(app.config["TREEBUILDERS"].keys()):
        inpath = Path(app.config["DATA"]) / familyname / method / RUN_LOG_NAME
...
def get_status(familyname, method):
    inpath = None
    if method in list(app.config["TREEBUILDERS"].keys()):
        inpath = Path(app.config["DATA"]) / familyname / method / STATUS_NAME
    elif method in list(app.config["ALIGNERS"].keys()):
